...
def test_change_username_success(client):
    ...
    token = create_access_token(data={"sub": "testuser_change"})
...
def test_change_username_to_same_username(client):
    ...
    token = create_access_token(data={"sub": "testuser_same"})
...
def test_old_jwt_invalid_after_username_change(client):
    ...
    # Get initial JWT
    old_token = create_access_token(data={"sub": "testuser_jwt"})
...
def test_login_with_new_username_after_change(client):
    ...
    # Change username
    old_token = create_access_token(data={"sub": "testuser_login"})
    response = client.put(
        "/auth/username",
